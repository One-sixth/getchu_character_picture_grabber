...
After a while, getchu will have new products on the shelves and need to update the database.  
The update method is to clear the completion flag first, and then repeat the first and second steps above to update the database.  

一段时间后，getchu会有新的产品上架，需要更新数据库。  
更新方式是先清除完成标志，然后再重复以上的第一第二步，更新数据库  
